...
const getAll = (reviewed, cb) => {
  db_conn.query(
    ...
    ${reviewed ? 'INNER' : 'LEFT' } JOIN user_reviews\
    ON res.resource_id=user_reviews.resource_id\
    ${reviewed ? 'INNER' : 'LEFT' } JOIN reviews\
    ON reviews.review_id=user_reviews.review_id\
    ${reviewed ? 'INNER' : 'LEFT' } JOIN users ON users.user_id=user_reviews.user_id\
